...
# ==============================================================================
# 通用放置检测 (检查 nova2 是否放入 KLT)
# ==============================================================================
def object_in_container(
        env: ManagerBasedRLEnv | DirectRLEnv,
        object_cfg: SceneEntityCfg = SceneEntityCfg("object"),   # nova2
        container_cfg: SceneEntityCfg = SceneEntityCfg("container"), # KLT_good 或 KLT_bad
        # 容器的有效范围 (Box check)
        x_range: tuple[float, float] = (-0.12, 0.12), 
        y_range: tuple[float, float] = (-0.155, 0.155),
        z_range: tuple[float, float] = (-0.10, 0.10), # 增加 Z 轴检查，必须在盒子高度内
    ...
    # (可选) 增加一个条件：机器人必须已经松手
    # 这里为了简单，只判断几何位置。真正的任务完成可能需要判断 gripper_open
...
def create_detection_zone_visualization(
    ...
    try:
        ...
        # 从容器路径提取环境命名空间
        # 例如: /World/envs/env_0/Scene/KLT_good -> /World/envs/env_0

...
from leisaac.assets.robots.xtrainer import (
...
def is_xtrainer_at_rest_pose(joint_pos: torch.Tensor, joint_names: list[str]) -> torch.Tensor:
    """
    Check if the robot is in the rest pose.
    """
    is_reset = torch.ones(joint_pos.shape[0], dtype=torch.bool, device=joint_pos.device)
    reset_pose_range = XTRAINER_FOLLOWER_REST_POSE_RANGE
    joint_pos = joint_pos / torch.pi * 180.0  # change to degree
    for joint_name, (min_pos, max_pos) in reset_pose_range.items():
        joint_idx = joint_names.index(joint_name)
        is_reset = torch.logical_and(is_reset, torch.logical_and(joint_pos[:, joint_idx] > min_pos, joint_pos[:, joint_idx] < max_pos))
...
def convert_leisaac_action_to_lerobot(action: torch.Tensor | np.ndarray, use_xtrainer: bool = False) -> np.ndarray:
    """
    ...
    if use_xtrainer:
        ...
        motor_limits = XTRAINER_FOLLOWER_MOTOR_LIMITS
    else:
...
def convert_lerobot_action_to_leisaac(action: torch.Tensor | np.ndarray, use_xtrainer: bool = False) -> np.ndarray:
    """
    ...
    if use_xtrainer:
        ...
        for idx, joint_name in enumerate(joint_limits):
            motor_limit_range = motor_limits[joint_name]
            joint_limit_range = joint_limits[joint_name]
            processed_radius = (action[:, idx] - motor_limit_range[0]) / (motor_limit_range[1] - motor_limit_range[0]) \

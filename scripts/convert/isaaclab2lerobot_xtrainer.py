...
def convert_isaaclab_to_lerobot(file_name):
    ...
    repo_id = 'sim_xtrainer/xtrainer_task1'   #为转换后数据文件的输出路径以及文件名
    robot_type = 'xtrainer_follower'
    fps = 30
    hdf5_root = '/home/dobot/merge_hdf5'   #为待转换的 HDF5 的文件路径
    # hdf5_root = './datasets'
    hdf5_files = [os.path.join(hdf5_root, file_name)]
    #下面为使用该数据的任务的一句话描述
    task = 'Put good parts into the blue box and dirty ones into the red box.'   #任务1，把好的零件放入蓝盒子，脏的零件放入红盒子。
    # task = 'Gather to the recycling area and cover with anti-static lids.'   #任务2，聚拢到回收区，并盖上防静电盖。
    # task = 'Insert the test tube on the desktop into the rack.'    #任务1，将桌面上的试管插入试管架。
    push_to_hub = False
...
if __name__ == '__main__':
    file_name="1_merged_data_105.hdf5"

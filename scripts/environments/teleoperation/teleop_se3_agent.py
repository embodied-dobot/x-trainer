...
settings.set_bool("/app/livestream/allow/no-stun-server", True)
# 如果需要，也可以在这里强制固定分辨率
...
def main():  # noqa: C901
    ...
    # ==========================================================================
    # [新增] 强制修改 TGS 求解器的最小迭代次数
    # ==========================================================================
    # [修改点]在这里导入并使用 PhysxSchema
    # ==========================================================================
    try:
        # 1. [新增] 延迟导入 pxr (一定要放在这里！)
        from pxr import PhysxSchema 
        # 1. 获取当前舞台
        stage = env.sim.stage
        # 2. 找到 PhysicsScene (Isaac Lab 默认通常创建在 /physicsScene)
        scene_prim_path = "/physicsScene" 
        scene_prim = stage.GetPrimAtPath(scene_prim_path)
        
        if scene_prim.IsValid():
            # 3. 使用 PhysxSchema API 强制设置属性
            physx_api = PhysxSchema.PhysxSceneAPI(scene_prim)
            
            # 设置最小位置迭代次数 (Minimum Position Iteration Count) 为 16
            physx_api.CreateMinPositionIterationCountAttr().Set(4)
            
            print(f"[INFO] 已强制将 '{scene_prim_path}' 的 Minimum Position Iteration Count 设置为 4")
        else:
            print(f"[WARNING] 未找到路径为 '{scene_prim_path}' 的 PhysicsScene，跳过设置。")
            
    except Exception as e:
        print(f"[ERROR] 设置物理参数时发生错误: {e}")
